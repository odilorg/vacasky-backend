...
{{-- SEO Meta Tags --}}
@section('title')
{{ $tag->name }} | Blog | Vacasky Travel
@endsection

@section('meta_description')
Browse all blog posts tagged with {{ $tag->name }}
@endsection
...
@section('content')
	...
	<section class="page-banner" style="background-image: url({{ asset('vacasky/images/background/15.jpg') }})">
		<div class="auto-container">
			...
			<h1 class="page-banner_title">TAG: {{ strtoupper($tag->name) }}</h1>
		</div>
	</section>
	<!-- End Page Banner -->
	...
	<section class="blog-grid-section">
		<div class="auto-container">
			<div class="section-title text-center">
				<h2>{{ $blogs->total() }} {{ Str::plural('Post', $blogs->total()) }} tagged with "{{ $tag->name }}"</h2>
			</div>
			...
			<div class="row clearfix">
									...
									@if($blog->tags && $blog->tags->count() > 0)
										<div class="blog-tags">
											@foreach($blog->tags->take(3) as $blogTag)
												<a href="{{ route('blog.tag', $blogTag->slug) }}" class="tag-badge {{ $blogTag->id === $tag->id ? 'active' : '' }}">{{ $blogTag->name }}</a>
											@endforeach
										</div>
									@endif
								</div>
				...
				@empty
					<div class="col-12">
						<div class="no-blogs">
							<h3>No blog posts found with this tag</h3>
